#include "Container.h"
#include "Logs.h"

Container::Container() {
    settings = new Settings();
    load();
}

Container::~Container() { 
    delete settings;
}

void Container::load() {
    boost::property_tree::ptree tree;

    std::ifstream file(config);

    if (!file.good()) {
        LOG_WARNING(PROTOCOLS::SETTINGS, "Config file not found. Creating default configuration.");
        create();

        return;
    }

    try {
        boost::property_tree::read_json(config, tree);
        settings->host = tree.get<std::string>("host", "127.0.0.1");
        settings->http_port = tree.get<uint16_t>("http_port", 8080);
        settings->websocket_port = tree.get<uint16_t>("websocket_port", 9090);
        settings->program_name = tree.get<std::string>("program_name", "MyApp");
        settings->db_host = tree.get<std::string>("db_host", "localhost");
        settings->db_port = tree.get<uint16_t>("db_port", 3306);
        settings->db_user = tree.get<std::string>("db_user", "root");
        settings->db_pass = tree.get<std::string>("db_pass", "");
        settings->version = tree.get<uint32_t>("version", 1);

        LOG_INFO(PROTOCOLS::SETTINGS, "Configuration loaded from {}.", config);
    }
    catch(std::exception& e) {
        LOG_FATAL(PROTOCOLS::SETTINGS, "Error loading config [{}]", e.what());
        create();
    }
}

void Container::create() {
    boost::property_tree::ptree tree;

    settings->host = "127.0.0.1";
    settings->http_port = 8080;
    settings->websocket_port = 9090;
    settings->program_name = "MyApp";
    settings->db_host = "localhost";
    settings->db_port = 3306;
    settings->db_user = "root";
    settings->db_pass  = "root";
    settings->version = 1;

    tree.put("host", settings->host);
    tree.put("http_port", settings->http_port);
    tree.put("websocket_port", settings->websocket_port);
    tree.put("program_name", settings->program_name);
    tree.put("db_host", settings->db_host);
    tree.put("db_port", settings->db_port);
    tree.put("db_user", settings->db_user);
    tree.put("db_pass", settings->db_pass);
    tree.put("version", settings->version);

    try {
        boost::property_tree::write_json(config, tree);
        LOG_INFO(PROTOCOLS::SETTINGS, "Default configuration file created {}.", config);
    }
    catch (std::exception& e) {
        LOG_FATAL(PROTOCOLS::SETTINGS, "Error writing default config [{}]", e.what());
    }
}